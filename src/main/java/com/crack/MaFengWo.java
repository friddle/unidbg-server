...
public class MaFengWo extends AbstractJni {
    ...
    static {
        //防止打成jar包的时候找不到文件
        String soPath = "./libmfw.so";
        String appPath = "./mafengwo.apk";
        ...
        try {
            ...
            Files.copy(inputStream, Paths.get("./tmp/libmfw.so"), StandardCopyOption.REPLACE_EXISTING);
            InputStream appinputStream = appPathResource.getInputStream();
            Files.copy(appinputStream, Paths.get("./tmp/mafengwo.apk"), StandardCopyOption.REPLACE_EXISTING);
...
//        emulator = new AndroidARMEmulator("com.mfw.roadbook"); // 创建模拟器实例，要模拟32位或者64位，在这里区分
        ...
        vm = emulator.createDalvikVM(new File("./tmp/mafengwo.apk")); // 创建Android虚拟机
        vm.setJni(this);
        //vm.setVerbose(true);// 设置是否打印Jni调用细节
        // 自行修改文件路径,loadLibrary是java加载so的方法
        DalvikModule dm = vm.loadLibrary(new File("./tmp/libmfw.so"), true); // 加载libmfw.so到unicorn虚拟内存，加载成功以后会默认调用init_array等函数
